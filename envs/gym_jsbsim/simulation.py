...
class Simulation(object):
    ...
    ROOT_DIR = os.path.dirname(os.path.abspath(jsbsim.__file__))
    ...
    def __init__(self,
        ...
        """
        self.jsbsim = jsbsim.FGFDMExec(root_dir=self.ROOT_DIR) # This root_dir is the root directory of the jsbsim
        ...
        self.initialise(self.sim_dt, self.aircraft, init_conditions)
    ...
    def __deepcopy__(self, memo=None, _nil=[]):
        ...
        obj_tmp = type(self)(self.sim_frequency_hz, self.aircraft, self.init_conditions)
        for key in obj_tmp.__dict__.keys():
            if key != 'jsbsim':
                obj_tmp.__setattr__(deepcopy(key, memo), deepcopy(self.__getattribute__(key), memo))
            else:
            #     obj_tmp.__setattr__(deepcopy(key, memo), self.__getattribute__(key))
                
                tmp_sim = jsbsim.FGFDMExec(root_dir=self.ROOT_DIR)
                tmp_sim.set_debug_level(0)
                tmp_sim = self._copy_initialise(tmp_sim, self.sim_dt, self.aircraft, deepcopy(self.init_conditions))
                # tmp_sim.initialise(self.sim_dt, self.aircraft, deepcopy(self.init_conditions))
                tmp_sim.disable_output()
                
                
                
                tmp_sim.set_aircraft_path(self.__getattribute__(key).get_aircraft_path())
                tmp_sim.set_debug_level(self.__getattribute__(key).get_debug_level())
                # tmp_sim.set_dt(self.__getattribute__(key).get_dt())
                tmp_sim.set_engine_path(self.__getattribute__(key).get_engine_path())
                # tmp_sim.set_logging_rate(self.__getattribute__(key).get_logging_rate())
                # tmp_sim.set_output_directive(self.__getattribute__(key).get_output_directive())
                # tmp_sim.set_output_filename(self.__getattribute__(key).get_output_filename())
                tmp_sim.set_output_path(self.__getattribute__(key).get_output_path())
                # tmp_sim.set_property_value(self.__getattribute__(key).get_property_value())
                tmp_sim.set_root_dir(self.__getattribute__(key).get_root_dir())
                tmp_sim.set_sim_time(self.__getattribute__(key).get_sim_time())
                tmp_sim.set_systems_path(self.__getattribute__(key).get_systems_path())
                tmp_sim.set_trim_status(self.__getattribute__(key).get_trim_status())
                
                obj_tmp.__setattr__(deepcopy(key, memo), tmp_sim)
        return obj_tmp
    
    def _copy_initialise(self, tmp_jsbsim, dt: float, model_name: str,
                   init_conditions: Dict['prp.Property', float] = None) -> None:
        """
        Loads an aircraft and initialises simulation conditions.
        ...
        ic_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ic_file)
        tmp_jsbsim.load_ic(ic_path, False)
                
        load_success = tmp_jsbsim.load_model(model_name)

        if not load_success:
            raise RuntimeError('JSBSim could not find specified model_name: '
                               + model_name)
        
        tmp_jsbsim.set_dt(dt)
        # extract set of legal property names for this aircraft
        # TODO: can translate the .split(" ")[0] once JSBSim bug has been fixed (in progress)
