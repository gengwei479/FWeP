...
class TRANSDATA(Dataset):
    def __init__(self, data_file, max_size, max_seq_length, obs_tokenizer_tool, act_tokenizer_tool, aircraft, llm, vab_size, train_ratio, flag = "train"):
        ...
        self.llm = llm
        self.flag = flag
        self.obs_data, self.act_data, self.total_obs_withouttoken_mrad, self.total_act_withouttoken_mrad = self.load_data(data_file, self.obs_tokenizer_tool, self.act_tokenizer_tool, vab_size, train_ratio)
...
class TRANSGPT():
    def __init__(self, args) -> None:
        ...
        self.train_data = TRANSDATA(os.path.join(os.getcwd(), self.args.master_data_dir, self.args.sear_version), max_size = self.args.max_dataset_size, max_seq_length = self.args.max_seq_length, 
                               ...
                               aircraft = self.args.aircraft[0], llm = self.args.kine_llm, vab_size = self.vab_size, train_ratio = args.train_ratio)
    ...
    def load_finetuned_model(self):
        if os.path.exists(os.path.join(self.args.finetuned_checkpoint, 'kine_llm_eval', self.args.kine_llm, self.args.aircraft[0] + '_token_' + self.args.sear_version)):
