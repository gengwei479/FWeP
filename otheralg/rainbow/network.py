...
class Dueling_Net(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = nn.Linear(args.DQN_state_dim, args.DQN_hidden_dim)
        self.fc2 = nn.Linear(args.DQN_hidden_dim, args.DQN_hidden_dim)
        if args.DQN_use_noisy:
            self.V = NoisyLinear(args.DQN_hidden_dim, 1)
            self.A = NoisyLinear(args.DQN_hidden_dim, args.DQN_action_dim)
        else:
            self.V = nn.Linear(args.DQN_hidden_dim, 1)
            self.A = nn.Linear(args.DQN_hidden_dim, args.DQN_action_dim)
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.fc2 = nn.Linear(args.DQN_hidden_dim, args.DQN_hidden_dim)
        if args.DQN_use_noisy:
            self.fc3 = NoisyLinear(args.DQN_hidden_dim, args.DQN_action_dim)
        else:
            self.fc3 = nn.Linear(args.DQN_hidden_dim, args.DQN_action_dim)
